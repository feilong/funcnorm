...
def test_compute_geodesic_distances():
    ...
    assert_equal(compute_geodesic_distances(np.array([[np.pi], [0.5]]),
                                            np.array([[0], [0.5]])),
                 np.pi)
    assert_equal(compute_geodesic_distances(np.array([[np.pi*0.5], [0.5+np.pi]]),
                                            np.array([[np.pi*0.5], [0.5]])),
                 np.pi)
    assert_almost_equal(compute_geodesic_distances(np.array([[np.pi], [0.5+np.pi]]),
                                            np.array([[np.pi], [0.5]])),
                 0)
    assert_almost_equal(compute_geodesic_distances(np.array([[0], [0.5+np.pi]]),
                                            np.array([[0], [0.5]])),
                 0)
...
def test_compare_with_matlab():
    ...
    for coord in ['cartesian', 'spherical']:
        for submode in ['single', 'multi']:
            for i in range(20):
                in_file = os.path.join(DIR, 'test_compute_geodesic_distances_data',
                                       '%s-%s-%03d.mat' % (coord, submode, i))
                data = loadmat(in_file)
                a, b = data['a'], data['b']
                out_file = os.path.join(DIR, 'test_compute_geodesic_distances_data',
